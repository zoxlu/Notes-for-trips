...
3. 針對每個素材，判斷類型：`place`（景點/商店）、`food`（餐廳/美食）、`transport`（交通）、`accommodation`（住宿）、`activity`（活動如健行、吉卜力公園）。一支影片或一篇文章可能同時提到多個地點，需要拆成多則筆記，並在每則筆記的來源註明是同一部影片的哪個段落（可用時間戳記）。
4. 每則筆記的 *原始連結備註* 這個區塊放上 `raw/` 來源檔案的 wikilink，並把該檔案裡面文字敘述部份到這裡（詳細格式見下方「原始連結備註」章節）。
...
8. **交叉連結（雙向檢查，避免全庫兩兩比對拖慢速度）**：
	   - 檢查這批新增/更新的筆記內文（不含 frontmatter），如果提到了 `wiki/` 裡其他任何筆記的**標題**或 **`aliases`** 裡列出的別名、且目前還不是 wikilink 形式，把它轉成 `[[該筆記標題]]`
	   - 反過來，也要檢查 `wiki/` 裡**所有既有筆記**的內文，如果提到了這批新增/更新筆記的標題或別名、且還不是 wikilink，一樣要補上連結（這樣才能處理「舊筆記提到新筆記」的情況）
	   - 只比對標題和明確列出的 `aliases`，**不要**自己發明關鍵字或憑語意猜測要連結什麼，避免把單純提到地名/人名當背景描述的句子過度連結，影響閱讀體驗
	   - 不要修改 `## 📝 我的備註` 區塊裡的內容
	   - **主動補充 `aliases`**：處理素材時，如果發現同一個地點/人物有多種常見稱呼（例如車站全名 vs 簡稱、店名的中日文版本、家人習慣用的暱稱），主動幫對應筆記的 `aliases` 補上，不用等使用者要求。既有筆記如果因為這次新素材而發現了新的別名寫法，也要一併補上該筆記的 `aliases`，這樣往後才能持續被交叉連結正確比對到。
