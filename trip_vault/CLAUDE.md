...
7. 全部處理完後，**重新產生** `wiki/1.分類瀏覽/` 底下所有 index 頁面、`wiki/7.繃皮/` 底下所有個人分享頁（見下方規則），確保連結清單是最新的。
8. 在回覆使用者時，簡短列出這次新增/更新了哪些筆記、交叉連結加了幾個、以及有沒有發現待確認的衝突。
9. **圖片檔名一致性檢查**：這批處理完的筆記，如果引用了 `04-Attachments/` 裡的本機圖片，檢查該筆記裡所有提到這個檔名的地方（`image` 欄位、內文嵌入語法等）是否完全一致，也確認檔名本身沒有空格。不一致或有空格的話，統一修正。
